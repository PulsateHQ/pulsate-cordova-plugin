...
public class PulsatePlugin extends CordovaPlugin {
    ...
    public boolean execute(String action, JSONArray args, final CallbackContext callbackContext) throws JSONException {
        ...
        switch (action) {
            case "setAuthData": {
                AuthorizationData data = new AuthorizationData(args.getString(0), args.getString(1));
            ...
            case "createAttributeWithString": {
            ...
            case "useInitialsForUserAvatar": {
                manager.useInitialsForUserAvatar(args.getBoolean(0));
                callbackContext.success();
                return true;
            }
            case "setSmallInAppNotificationDuration": {
                Object value = args.get(0);
                if (value instanceof Integer)
                    manager.setSmallInAppNotificationDuration((Integer) value);
                callbackContext.success();
                return true;
            }
            case "setOnInboxRightButtonClickListenerAndroid": {
                manager.setOnInboxRightButtonClickListener(new View.OnClickListener() {
                    @Override
                    public void onClick(View v) {
                        PluginResult result = new PluginResult(PluginResult.Status.OK, "onClick");
            ...
            case "removeOnInboxRightButtonClickListenerAndroid": {
                manager.setOnInboxRightButtonClickListener(null);
                callbackContext.success();
                return true;
            }
            case "setUserUnauthorizedListenerAndroid": {
                manager.setUserUnauthorizedListener(new IPulsateUserUnauthorizedListener() {
                    @Override
                    public void onUnauthorizedAction() {
                        PluginResult result = new PluginResult(PluginResult.Status.OK, "onUnauthorizedAction");
                        result.setKeepCallback(true);
                        callbackContext.sendPluginResult(result);
                    }
                });
            ...
            case "setUnreadCountUpdateListenerAndroid": {
